<body>
    <footer>
        <article class="footer-content w-70 padding-50y">
            <img src="media/images/logo_confiz.png" width="auto" height="200" alt="">
            <div>
                <h2 class="p-30">Nos réseaux</h2>
                <span>
                    <a href=""><i class="fa-brands fa-square-facebook"></i></a>
                    <a href=""><i class="fa-brands fa-square-x-twitter"></i></a>
                    <a href=""><i class="fa-brands fa-instagram"></i></a>
                </span>   
            </div>
            <div>
                <h2 class="p-30">Légal</h2>
                <a href="">Mentions légales</a>
                <a href="">Politiques de confidentialité</a>
                <a href="">Remboursements</a>
                <h2 class="p-30">Sitemap</h2>
                <a href="">Accueil</a>
                <a href="">Boutiques</a>
                <a href="">Connexion</a>
            </div>
            <div>
                <h2 class="p-30">Sitemap</h2>
                <a href="index.php#notre_histoire">Notre histoire</a>
                <a href="">Nos collaborateurs</a>
                <a href="">Notre équipe</a>
            </div>
        </article>
    </footer>
</body>